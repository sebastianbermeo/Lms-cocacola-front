...
export default function CrearLeccion({ lecciones = [], setLecciones = () => { }, modulos = [] }) {
  ...
  const leccionesFiltradas =
    filtroModulo === 'Todos'
      ? lecciones
      : lecciones.filter((l) => l.modulo === filtroModulo)
  ...
  const guardarLeccion = () => {
    if (!form.titulo.trim() || !form.descripcion.trim() || !form.imagen.trim() || !form.contenidoTexto.trim() || !form.modulo) {
      alert('Por favor, completa todos los campos obligatorios.')
      return
    }

    if (editando) {
      setLecciones((prev) => prev.map((l) => (l.id === form.id ? { ...form } : l)))
      setEditando(false)
    } else {
      setLecciones([...lecciones, { id: Date.now(), ...form }])
    }
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }}>
      ...
      {/* ==================== SECCIÓN 1: DATOS DE LA TARJETA ==================== */}
      <div className="bg-white border border-gray-200 rounded-xl shadow-sm p-6 mb-8">
        <h3 className="text-lg font-semibold text-[#F40009] mb-4">
          🧩 Datos de la tarjeta
        </h3>
      ...
      {/* ==================== SECCIÓN 2: CONTENIDO DE LA LECCIÓN ==================== */}
      ...
      {/* Tabla */}
      <TablaLecciones
        lecciones={leccionesFiltradas}
        onEdit={(row) => editarLeccion(row)}
        onDelete={(row) => eliminarLeccion(row.id)}
      />
    </motion.div>
  )
}