...
import { Plus, Save, RotateCcw, Trash2, FileQuestion, ClipboardList } from 'lucide-react'
...
export default function CrearQuiz({ lecciones = [], onGuardar, quizEditando, onCancelarEdicion }) {
  ...
  useEffect(() => {
    if (quizEditando) {
      setForm({
        leccionId: quizEditando.leccion?.id || quizEditando.leccionId || '',
        minCorrectas: quizEditando.minCorrectas || 1,
        preguntas:
          quizEditando.preguntas?.map((p) => ({
            texto: p.texto,
      ...
      setEditando(false)
      setForm({ leccionId: '', minCorrectas: 1, preguntas: [{ texto: '', opciones: ['', ''], correcta: 0 }] })
    }
  }, [quizEditando])
    ...
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }}>
      ...
      <div className="bg-white border border-gray-200 rounded-xl shadow-sm p-6 mb-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">
              Respuestas mínimas para aprobar <span className="text-[#F40009]">*</span>
            </label>
            <input
              type="number"
              min={1}
      ...
      <div className="bg-white border border-gray-200 rounded-xl shadow-sm p-6 mb-6">
          ...
          <div key={index} className="border border-gray-200 p-4 rounded-lg mb-6 bg-gray-50 relative shadow-sm">
            ...
            {p.opciones.map((op, iOpcion) => (
              <div key={iOpcion} className="flex items-center space-x-2 mb-2">
                ...
                <input
                  type="text"
                  value={typeof op === 'string' ? op : op.texto}
        ...
        <div className="flex justify-between items-center mt-4">
          <button onClick={agregarPregunta} className="flex items-center text-[#F40009] font-medium">
